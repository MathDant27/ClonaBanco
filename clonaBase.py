...
if not jpype.isJVMStarted():
    jpype.startJVM(
        jvm_path,
        f"-Djava.class.path={jdbc_driver_path}{os.pathsep}{bson_driver_path}",
        convertStrings=True,
    )

# Parâmetros de conexão do servidor de cópia
copia_config = {
    "servidor": "IP_SERVIDOR_COPIA",
    "porta": "PORTA_COPIA",
    "banco": "NOME_BANCO_COPIA",
    "usuario": "USUARIO_COPIA",
    "senha": "SENHA_COPIA",
    "informixserver": "SERVIDOR_COPIA",
}

# Parâmetros de conexão do servidor de destino
destino_config = {
    "servidor": "IP_SERVIDOR_DESTINO",
    "porta": "PORTA_DESTINO",
    "banco": "NOME_BANCO_DESTINO",
    "usuario": "USUARIO_DESTINO",
    "senha": "SENHA_DESTINO",
    "informixserver": "SERVIDOR_DESTINO",
}
...
def conectar_db(config):
    ...
    try:
        ...
        logging.info(
            f"Conexão estabelecida com sucesso ao servidor {config['servidor']}"
        )
        return conn
    except Exception as e:
        logging.error(f"Erro ao conectar ao servidor {config['servidor']}: {str(e)}")
        estatisticas["erros"].append(
            f"Erro ao conectar ao servidor {config['servidor']}: {str(e)}"
        )
        raise
...
def obter_tabelas_para_clonar(conn_copia, conn_destino):
    ...
    try:
        ...
        cursor_copia.execute(
            f"SELECT tabname FROM systables WHERE tabtype = 'T' AND owner = 'informix' AND tabname IN ({placeholders})"
        )
        tabelas_copia = [row[0] for row in cursor_copia.fetchall()]

        if not tabelas_copia:
            logging.warning(
                "Nenhuma das tabelas especificadas foi encontrada no ambiente de cópia!"
            )
            return []
...
def gerar_hash_registro(registro):
    ...
    valores_str = "|".join(
        str(valor if valor is not None else "NULL") for valor in registro
    )
...
def obter_ultimos_registros(conn, tabela, limite=100):
    ...
    try:
        ...
        estatisticas["erros"].append(
            f"Erro ao obter registros da tabela {tabela}: {str(e)}"
        )
        return [], []
    finally:
        cursor.close()
...
def verificar_registros_existentes(conn, tabela, dados, colunas):
    ...
    try:
        ...
        hashes_destino = [
            gerar_hash_registro(registro) for registro in registros_destino
        ]

        # Filtra apenas os registros que não existem no destino
        novos_dados = [
            dados[i]
            for i, hash_valor in enumerate(hashes_origem)
            if hash_valor not in hashes_destino
        ]
        ...
        logging.error(
            f"Erro ao verificar registros existentes na tabela {tabela}: {str(e)}"
        )
        estatisticas["erros"].append(
            f"Erro ao verificar registros existentes na tabela {tabela}: {str(e)}"
        )
...
def inserir_dados(conn, tabela, dados, colunas):
    ...
    try:
        ...
        estatisticas["erros"].append(
            f"Erro ao inserir dados na tabela {tabela}: {str(e)}"
        )
...
def excluir_dados_antigos(conn, tabelas):
    """Exclui registros com mais de 6 meses do banco de destino"""
    print("\n=== EXCLUSÃO DE DADOS ANTIGOS ===")
    print(
        "Esta operação excluirá registros com mais de 6 meses das tabelas do banco de destino."
    )

    # Solicita confirmação do usuário
    confirmacao = input("Deseja prosseguir com a exclusão? (S/N): ").strip().upper()
    if confirmacao != "S":
        print("Operação de exclusão cancelada pelo usuário.")
        logging.info("Operação de exclusão cancelada pelo usuário.")
        return
    ...
    try:
        for tabela in tabelas:
            try:
                # Verifica se a tabela tem uma coluna de data
                ...
                colunas_data = [
                    col
                    for col in colunas
                    if "DATA" in col or "DATE" in col or "DT_" in col
                ]

                if not colunas_data:
                    logging.warning(
                        f"Tabela {tabela} não possui colunas de data identificáveis. Exclusão ignorada."
                    )
                    print(
                        f"Tabela {tabela}: Sem colunas de data identificáveis. Exclusão ignorada."
                    )
                    continue
                ...
                # Conta registros a serem excluídos
                cursor.execute(
                    f"SELECT COUNT(*) FROM {tabela} WHERE {coluna_data} < '{data_limite_str}'"
                )
                qtd_registros = cursor.fetchone()[0]

                if qtd_registros == 0:
                    print(f"Tabela {tabela}: Nenhum registro antigo encontrado.")
                    continue

                print(
                    f"Tabela {tabela}: {qtd_registros} registros antigos encontrados."
                )
                ...
                cursor.execute("BEGIN WORK")
                cursor.execute(
                    f"DELETE FROM {tabela} WHERE {coluna_data} < '{data_limite_str}'"
                )
                cursor.execute("COMMIT WORK")

                logging.info(
                    f"Excluídos {qtd_registros} registros antigos da tabela {tabela}"
                )
                print(
                    f"Tabela {tabela}: {qtd_registros} registros excluídos com sucesso."
                )

                total_excluidos += qtd_registros
                estatisticas["registros_excluidos"] += qtd_registros
                ...
                try:
                    ...
                    pass
                logging.error(
                    f"Erro ao excluir registros antigos da tabela {tabela}: {str(e)}"
                )
                estatisticas["erros"].append(
                    f"Erro ao excluir registros antigos da tabela {tabela}: {str(e)}"
                )
                print(f"Erro ao excluir registros da tabela {tabela}: {str(e)}")
    except Exception as e:
        logging.error(f"Erro durante o processo de exclusão: {str(e)}")
        estatisticas["erros"].append(f"Erro durante o processo de exclusão: {str(e)}")
    finally:
        try:
            conn.jconn.setAutoCommit(True)
        except:
    ...
    print(f"\nTotal de registros excluídos: {total_excluidos}")
    logging.info(f"Total de registros excluídos: {total_excluidos}")
...
def copiar_tabela(tabela, conn_copia, conn_destino):
    ...
    try:
        ...
        novos_dados = verificar_registros_existentes(
            conn_destino, tabela, dados, colunas
        )
...
Registros excluídos (mais de 6 meses): {estatisticas['registros_excluidos']}
...
"""
    ...
    print(f"Registros excluídos: {estatisticas['registros_excluidos']}")
...
def main():
    ...
    try:
        ...
        if not tabelas_para_clonar:
            print(
                "Nenhuma tabela disponível para clonar. Verifique os nomes das tabelas."
            )
        else:
            ...
            # Executa a exclusão de dados antigos no banco de destino
            excluir_dados_antigos(conn_destino, tabelas_para_clonar)
